...
function check_raylib()
    if(os.isdir("raylib") == false and os.isdir("raylib") == false) then
        if(not os.isfile("rl_.zip")) then
            print("Raylib not found, downloading from github")
            local result_str, response_code = http.download("https://github.com/raysan5/raylib/archive/refs/heads/master.zip", "rl_.zip", {
                progress = download_progress,
                headers = { "From: Premake", "Referer: Premake" }
            })
        end
        print("Unzipping to " ..  os.getcwd())
        zip.extract("rl_.zip", os.getcwd())
		os.remove("rl_.zip")
		os.rename("raylib-master", "raylib");
    end
end
